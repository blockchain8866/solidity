...
void RedundantStoreEliminator::operator()(Block& _block)
{
	...
	for (size_t i = 0; i < _block.statements.size(); ++i)
	{
		...
		if (holds_alternative<ExpressionStatement>(statement))
		{
			...
			if (auto vars = isSimpleStore(evmasm::Instruction::SSTORE, exprStatement))
			{
				if (m_storage.values.count(vars->first) && latestStore.count(vars->first))
	...
	for (size_t i = 0; i < _block.statements.size(); ++i)
	{
		if (!redundantStores.count(i))
			// TODO we have to retain the side-effects of the arguments.
